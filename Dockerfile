...
# Передаём секреты для сборки через ARG (значения из GitHub Secrets)
...
# Выполняем сборку приложения (например, Next.js и Payload)
RUN pnpm build
...
# но **не копируем** переменные окружения для секретов.
COPY --from=builder /app/.next ./.next
COPY --from=builder /app/public ./public

# Если для воркеров нужны отдельные файлы, копируем их тоже:
COPY --from=builder /app/workers.ts ./workers.ts
COPY --from=builder /app/shared/workers ./shared/workers

# Передача всех переменных окружения для продакшена
ARG NEXT_PUBLIC_SERVER_URL
ARG NEXT_PUBLIC_BASE_URL
ARG DATABASE_URI
ARG PAYLOAD_SECRET
