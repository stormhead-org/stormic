...
const isRole = (role: number | HostRole): role is HostRole => {
	return (
		typeof role === 'object' &&
		role !== null &&
		'id' in role &&
...
export const EditorForm: React.FC<Props> = ({
	...
	const sortedRoles = React.useMemo(() => {
		...
		const orderedRoles = roleOrder
			...
			.filter((role): role is HostRole => !!role)
	...
	useEffect(() => {
		...
		if (
			roleOrder.length !== initialIds.size ||
			roleOrder.some(id => !initialIds.has(id))
		) {
