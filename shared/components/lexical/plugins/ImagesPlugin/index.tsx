...
export function InsertImageDialog({
	...
	const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		try {
			...
			const uploadedSrc = result.doc.url // Предполагается, что createMedia возвращает объект с полем url
			setSrc(uploadedSrc)
			...
			<DialogContent>
				...
				{!mode && (
					<div className='flex space-x-2'>
						...
						<Button onClick={() => setMode('file')}>Файл</Button>
					</div>
				)}
				{mode === 'url' && (
...
export default function ImagesPlugin(): JSX.Element | null {
	...
	useEffect(() => {
		if (!editor.hasNodes([ImageNode])) {
			throw new Error('ImagesPlugin: ImageNode не зарегистрирован в редакторе')
		}
