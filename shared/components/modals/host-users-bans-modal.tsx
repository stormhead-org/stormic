...
export const HostUsersBansModal: React.FC<Props> = ({
	...
	const banUser = async (userId: number) => {
		try {
			const req = await fetch('/api/hostUsersBans', {
				method: 'POST',
				credentials: 'include',
	...
	const handleSubmitBanUsers = async (userIds: number[]) => {
		try {
			...
			await Promise.all(newUsersToBan.map(userId => banUser(userId)))
			setSelectedUsers([])
			handleClose()
			router.refresh()
		} catch (err) {
	...
	const handleBanSelectedUsers = async () => {
		if (selectedUsers.length > 0) {
			await handleSubmitBanUsers(selectedUsers)
		}
	}
