...
export default async function handler(
	...
	try {
		...
		if (!conversationId) {
			return res.status(400).json({ error: 'Conversation ID не найдено' })
		}
		...
		if (!conversation) {
			return res.status(404).json({ error: 'Conversation не найдено' })
		}
		...
		if (!directMessage || directMessage.deleted) {
			return res.status(404).json({ error: 'Message не найдено' })
		}
